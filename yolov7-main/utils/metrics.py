...
def plot_mc_curve(px, py, save_dir='mc_curve.png', names=(), xlabel='Confidence', ylabel='Metric'):
    ...
    # 创建一个空字典用于保存数据
    data_dict = {'px': px.tolist()}

    if 0 < len(names) < 21:  # 如果类别数量小于21，显示每个类别的图例
        for i, y in enumerate(py):
            ax.plot(px, y, linewidth=1, label=f'{names[i]}')  # 绘制曲线
            data_dict[names[i]] = y.tolist()  # 将每个类别的纵坐标值保存到字典中
    else:
        ax.plot(px, py.T, linewidth=1, color='grey')  # 绘制曲线

    y = py.mean(0)
    ax.plot(px, y, linewidth=3, color='blue', label=f'all classes {y.max():.2f} at {px[y.argmax()]:.3f}')

    # 将所有类别的平均值保存到字典中
    data_dict['all'] = y.tolist()
    ...
    # 将数据字典保存为CSV文件
    dataformat = pd.DataFrame(data_dict)
    save_csvpath = Path(save_dir).with_suffix('.csv')  # 定义csv文件的保存位置
    dataformat.to_csv(save_csvpath, sep=',')
