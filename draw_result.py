...
if __name__ == '__main__':
    ...
    result_dict = {
        'YOLOv7': r'D:\A_Python\YOLO_RAW\yolov7-main\runs\train\exp\results.txt',
        'YOLOv7_ours': r'D:\A_Python\YOLO_RAW\yolov7-ours\runs\train\exp\results.txt',
    }
    ...
    plt.grid()
    # 显示图像
    plt.savefig("P_result.png", dpi=600)
    plt.show()
    ...
    for modelname in result_dict:
        ...
        if ext == 'csv':
            ...
            with open(res_path, 'r') as f:
                ...
                data = []
                for d in datalist:
                    data.append(float(d.strip().split()[9]))
                data = np.array(data)
        x = range(len(data))
        plt.plot(x, data, label=modelname, linewidth='1')
    ...
    plt.grid()
    # 显示图像
    plt.savefig("R_result.png", dpi=600)
    plt.show()